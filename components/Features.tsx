...
const FeatureCard: React.FC<FeatureCardProps> = ({ icon, title, description }) => {
  return (
    <div className="relative bg-[#020202]/30 backdrop-blur-sm p-6 rounded-xl gradient-border h-full group overflow-hidden">
        {/* Hover glow effect */}
        <div className="absolute top-0 left-0 w-full h-full bg-green-500/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-xl scale-150 blur-2xl"></div>
  ...
  return (
    <section id="features" ref={sectionRef} className="py-20 sm:py-24">
      <div className={`text-center mb-12 transition-all duration-700 ease-out ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
        ...
        <p className="mt-4 text-lg text-gray-400">Diseñado por y para precursores.</p>
      </div>
